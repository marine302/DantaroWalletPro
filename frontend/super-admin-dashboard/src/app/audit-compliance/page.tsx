...
const TabsTrigger = ({ value, children, activeTab, setActiveTab }: { value: string; children: React.ReactNode; activeTab?: string; setActiveTab?: (tab: string) => void }) => (
  <button
    className={`flex-1 px-3 py-2 text-sm font-medium rounded-md transition-colors ${
      activeTab === value ? 'bg-background shadow-sm' : 'hover:bg-background/60'
    }`}
...
const AuditCompliancePage = () => {
  ...
  const fetchAuditEvents = async () => {
    try {
      // const response = await fetch('/api/v1/audit_compliance/events');
  ...
  const fetchSuspiciousActivities = async () => {
    try {
      // const response = await fetch('/api/v1/audit_compliance/suspicious-activities');
      // const data = await response.json();
  ...
  const fetchComplianceMetrics = async () => {
    try {
      // const response = await fetch('/api/v1/audit_compliance/metrics');
      // const data = await response.json();
      // setMetrics(data);
    } catch (error) {
