...
def test_energy_rental_service():
    ...
    try:
        ...
        # 1. 에너지 풀 생성
        from app.models.energy_pool import EnergyPoolStatus
        ...
        print(
            f"✓ 사용 통계 조회: {stats['total_energy_used']} energy, ${stats['total_cost']}"
        )
        ...
        print(
            f"✓ 청구서 생성: {billing_record.invoice_number}, ${billing_record.final_amount}"
        )
        ...
        print(
            f"✓ 시스템 상태: {system_status['total_energy']} total energy, {system_status['utilization_rate']}% utilization"
        )
