...
async def test_audit_service():
    ...
    try:
        ...
        # 감사 로그 조회
        logs = await audit_service.get_audit_logs(entity_type="transaction", limit=5)
...
async def test_compliance_service():
    ...
    try:
        ...
        aml_result = await compliance_service.perform_aml_check(
            ...
            transaction_data={"destination": "외부계좌", "purpose": "개인송금"},
        )

        print(
            f"✅ AML 체크 완료: 상태 {aml_result.status}, 위험도 {aml_result.risk_level}"
        )

        # 제재 목록 체크 테스트
        sanctions_result = await compliance_service.check_sanctions_list(
            name="홍길동", nationality="KR"
        )
        ...
        pep_result = await compliance_service.check_pep_status(
            name="김정치", position="장관", country="KR"
        )

        print(
            f"✅ PEP 체크 완료: 상태 {pep_result.status}, 위험도 {pep_result.risk_level}"
        )
...
async def test_ml_anomaly_service():
    ...
    try:
        ...
        if anomaly_result:
            print(
                f"✅ 트랜잭션 이상 탐지: {anomaly_result.pattern_name}, 신뢰도 {anomaly_result.confidence_score}"
            )
        else:
            print("✅ 트랜잭션 정상 (이상 없음)")
        ...
        behavior_anomalies = await ml_service.detect_user_behavior_anomalies(
            user_id=1, time_window_days=30
        )

        print(
            f"✅ 사용자 행동 이상 탐지 완료: {len(behavior_anomalies)}개 이상 패턴 발견"
        )
