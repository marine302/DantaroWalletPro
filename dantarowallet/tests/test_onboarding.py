...
async def test_onboarding_system():
    ...
    async for db in get_db():
        ...
        try:
            ...
            onboarding = await service.create_onboarding(test_partner_id, configuration_data)
            ...
            if status:
                ...
                print(f"   현재 단계: {safe_int(status.current_step)}/{safe_int(status.total_steps)}")
            else:
                print("❌ 상태 조회 실패")
                return
            ...
            if final_status:
                ...
                print(f"현재 단계: {final_status.current_step}/{final_status.total_steps}")
