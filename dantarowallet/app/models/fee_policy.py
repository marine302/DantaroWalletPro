...
class PartnerFeePolicy(Base):
    ...
    internal_transfer_fee_rate = Column(
        Numeric(5, 4), default=0, comment="내부 이체 수수료율"
    )
    ...
    # 수익 분배
    platform_share_rate = Column(
        Numeric(5, 4), default=0.3, comment="플랫폼 수수료 분배율 (30%)"
    )
...
class FeeTier(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    fee_policy_id = Column(
        Integer, ForeignKey("partner_fee_policies.id"), nullable=False
    )
...
class PartnerEnergyPolicy(Base):
    ...
    default_policy = Column(
        Enum(EnergyPolicy), default=EnergyPolicy.WAIT_QUEUE, comment="기본 대응 정책"
    )

    # TRX 직접 결제 설정
    trx_payment_enabled = Column(Boolean, default=True, comment="TRX 결제 활성화")
    trx_payment_markup = Column(
        Numeric(5, 4), default=0.1, comment="TRX 결제 마크업 (10%)"
    )
    ...
    energy_saving_threshold = Column(
        Integer, default=20, comment="절약 모드 임계값 (%)"
    )
...
class UserTier(Base):
    ...
    withdrawal_limit_multiplier = Column(
        Numeric(5, 2), default=1.0, comment="출금 한도 배수"
    )
