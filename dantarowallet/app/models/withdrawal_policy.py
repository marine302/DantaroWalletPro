...
class WithdrawalApprovalRule(Base):
    ...
    # 정책 연결
    policy_id = Column(
        Integer, ForeignKey("partner_withdrawal_policies.id"), nullable=False
    )
...
class WithdrawalWhitelist(Base):
    ...
    # 관계 설정
    policy = relationship(
        "PartnerWithdrawalPolicy", back_populates="whitelist_addresses"
    )

    def __repr__(self) -> str:
        return (
            f"<WithdrawalWhitelist(address={self.address}, label={self.address_label})>"
        )
...
class WithdrawalRiskScore(BaseModel):
    ...
    withdrawal_id = Column(
        Integer, ForeignKey("withdrawals.id"), nullable=False, unique=True
    )
    ...
    recommended_action = Column(
        String(50), nullable=False, default="approve"
...
Index(
    "idx_withdrawal_whitelist_policy_address",
    WithdrawalWhitelist.policy_id,
    WithdrawalWhitelist.address,
)
Index(
    "idx_withdrawal_batch_log_policy_batch",
    WithdrawalBatchLog.policy_id,
    WithdrawalBatchLog.batch_id,
)
