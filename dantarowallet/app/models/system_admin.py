"""시스템 모니터링 및 관리 관련 모델"""
from sqlalchemy import Column, Integer, String, Boolean, DateTime, Text, Numeric, JSON, Date, ForeignKey
...
class SystemMonitoring(Base):
    ...
    partner_id = Column(String(36), ForeignKey("partners.id", ondelete="CASCADE"), comment="파트너 ID (NULL이면 전체)")
...
class SystemAlert(Base):
    ...
    partner_id = Column(String(36), ForeignKey("partners.id", ondelete="CASCADE"), comment="관련 파트너 ID")
...
class SuperAdminActivityLog(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    admin_user_id = Column(Integer, ForeignKey("super_admin_users.id", ondelete="CASCADE"), nullable=False, comment="슈퍼 어드민 사용자 ID")
...
class PlatformRevenueStatistics(Base):
    ...
    total_partner_commissions = Column(Numeric(18, 8), default=0, comment="총 파트너 커미션")
