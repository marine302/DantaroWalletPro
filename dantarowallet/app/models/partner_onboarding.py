...
class PartnerOnboarding(Base):
    ...
    manual_approval_required = Column(Boolean, nullable=False, default=False)  # 수동 승인 필요
    ...
    checklist = relationship("OnboardingChecklist", back_populates="onboarding", cascade="all, delete-orphan")
    logs = relationship("OnboardingLog", back_populates="onboarding", cascade="all, delete-orphan")

    def __repr__(self) -> str:
        return f"<PartnerOnboarding(partner_id={self.partner_id}, status={self.status})>"
...
class OnboardingStep(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    # 온보딩 연결
    onboarding_id = Column(Integer, ForeignKey("partner_onboardings.id"), nullable=False)
    
    # 단계 정보
    step_number = Column(Integer, nullable=False)
    step_name = Column(String(100), nullable=False)
    step_description = Column(Text, nullable=True)
    status = Column(SQLEnum(OnboardingStepStatus), nullable=False, default=OnboardingStepStatus.PENDING)
