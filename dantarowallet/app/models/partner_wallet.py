...
class PartnerWallet(Base):
    ...
    partner_id = Column(
        String(36), ForeignKey("partners.id"), nullable=False, comment="파트너사 ID"
    )
    wallet_type = Column(Enum(WalletType), nullable=False, comment="지갑 유형")
    wallet_address = Column(
        String(42), nullable=False, unique=True, comment="지갑 주소"
    )
...
class WalletTransaction(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    wallet_id = Column(
        Integer, ForeignKey("partner_wallets.id"), nullable=False, comment="지갑 ID"
    )
    ...
    # 상태 관리
    status = Column(
        Enum(TransactionStatus), default=TransactionStatus.PENDING, comment="상태"
    )
