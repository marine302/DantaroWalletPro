...
class HDWalletMaster(Base):
    ...
    partner_id = Column(
        String(36), ForeignKey("partners.id"), nullable=False, unique=True
    )
    ...
    collection_address = Column(
        String(42), nullable=True, comment="마스터 수집 주소"
    )  # 추가

    # 파생 정보
    derivation_path = Column(
        String(100), default="m/44'/195'/0'/0", comment="TRON 파생 경로"
    )
...
class UserDepositAddress(Base):
    ...
    # 주소 정보
    address = Column(
        String(42), nullable=False, unique=True, index=True, comment="TRON 입금 주소"
    )
    derivation_index = Column(Integer, nullable=False, comment="HD Wallet 파생 인덱스")
    encrypted_private_key = Column(
        String(500), nullable=False, comment="암호화된 개인키"
    )
    ...
    deposits = relationship(
        "Deposit",
        foreign_keys="[Deposit.to_address]",
...
class SweepConfiguration(Base):
    ...
    # Sweep 대상 지갑
    destination_wallet_id = Column(
        Integer, ForeignKey("partner_wallets.id"), nullable=False
    )
    ...
    min_sweep_amount = Column(
        Numeric(18, 6), default=10, comment="최소 Sweep 금액 (USDT)"
    )
    max_sweep_amount = Column(
        Numeric(18, 6), comment="최대 Sweep 금액 (제한없으면 NULL)"
    )

    # 스케줄 설정
    sweep_interval_minutes = Column(Integer, default=60, comment="Sweep 간격 (분)")
    immediate_threshold = Column(
        Numeric(18, 6), default=1000, comment="즉시 Sweep 임계값"
    )
    daily_sweep_time = Column(String(5), comment="일일 Sweep 시간 (HH:MM)")

    # 가스비 설정
    max_gas_price_sun = Column(
        Numeric(20, 0), default=1000, comment="최대 가스비 (SUN)"
    )
...
class SweepLog(Base):
    ...
    configuration_id = Column(
        Integer, ForeignKey("sweep_configurations.id"), nullable=False
    )
    deposit_address_id = Column(
        Integer, ForeignKey("user_deposit_addresses.id"), nullable=False
    )

    # Sweep 정보
    sweep_type = Column(
        String(20), default="auto", comment="Sweep 유형 (auto/manual/emergency)"
    )
    ...
    # 상태 관리
    status = Column(
        String(20), default="pending", comment="상태 (pending/confirmed/failed)"
    )
...
class SweepQueue(Base):
    ...
    # 큐 정보
    queue_type = Column(
        String(20), default="normal", comment="큐 유형 (normal/priority/emergency)"
    )
    ...
    status = Column(
        String(20),
        default="queued",
        comment="상태 (queued/processing/completed/failed)",
    )
