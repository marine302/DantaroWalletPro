...
class PartnerEnergyPool(Base):
    ...
    frozen_for_bandwidth = Column(
        Numeric(18, 6), default=0, comment="대역폭용 동결 TRX"
    )

    # 상태 및 예측
    status = Column(
        Enum(EnergyStatus), default=EnergyStatus.SUFFICIENT, comment="현재 상태"
    )
    ...
    # 관계 설정
    alerts = relationship(
        "EnergyAlert", back_populates="energy_pool", cascade="all, delete-orphan"
    )
    partner_usage_logs = relationship(
...
class EnergyAlert(Base):
    ...
    energy_pool_id = Column(
        Integer,
        ForeignKey("partner_energy_pools.id"),
        nullable=False,
        comment="에너지 풀 ID",
    )
...
class EnergyPrediction(Base):
    ...
    prediction_date = Column(
        DateTime(timezone=True), nullable=False, comment="예측 날짜"
    )
