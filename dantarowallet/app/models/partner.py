...
class Partner(Base):
    ...
    contact_email = Column(
        String(255), nullable=False, unique=True, comment="연락처 이메일"
    )
    ...
    api_usage_logs = relationship(
        "PartnerApiUsage", back_populates="partner", cascade="all, delete-orphan"
    )
    daily_statistics = relationship(
        "PartnerDailyStatistics", back_populates="partner", cascade="all, delete-orphan"
    )
    energy_allocations = relationship(
        "PartnerEnergyAllocation",
        back_populates="partner",
        cascade="all, delete-orphan",
    )
    partner_energy_usage_history = relationship(
    ...
    partner_fee_config_history = relationship(
        "PartnerFeeConfigHistory",
        back_populates="partner",
        cascade="all, delete-orphan",
    )
    onboarding_steps = relationship(
        "PartnerOnboardingStep", back_populates="partner", cascade="all, delete-orphan"
    )
    deployments = relationship(
        "PartnerDeployment", back_populates="partner", cascade="all, delete-orphan"
    )

    # 외부 지갑 관계 (TronLink 연동)
    wallets = relationship(
        "PartnerWallet", back_populates="partner", cascade="all, delete-orphan"
    )

    # Doc-26: 수수료 및 정책 관계
    fee_policy = relationship(
        "PartnerFeePolicy",
        back_populates="partner",
        uselist=False,
        cascade="all, delete-orphan",
    )
    withdrawal_policy = relationship(
        "PartnerWithdrawalPolicy",
        back_populates="partner",
        uselist=False,
        cascade="all, delete-orphan",
    )
    energy_policy = relationship(
        "PartnerEnergyPolicy",
        back_populates="partner",
        uselist=False,
        cascade="all, delete-orphan",
    )
    user_tiers = relationship(
        "UserTier", back_populates="partner", cascade="all, delete-orphan"
    )
    fee_calculation_logs = relationship(
        "FeeCalculationLog", back_populates="partner", cascade="all, delete-orphan"
    )
    policy_calculation_logs = relationship(
