...
async def get_providers(
    ...
    try:
        ...
        return ProvidersListResponse(success=True, data=providers)
...
async def get_provider_detail(
    ...
    try:
        ...
        return ProviderDetailResponse(success=True, data=provider)
...
async def get_market_summary(
    ...
    try:
        ...
        return MarketSummaryResponse(success=True, data=summary)
...
async def create_order(
    ...
    try:
        ...
        return CreateOrderResponse(success=True, data=order)
        ...
        for provider in providers:
            ...
            if price_info:
                provider_info.update(
                    {"current_price": price_info[0], "available_energy": price_info[1]}
                )
...
async def purchase_energy_multi_provider(
    ...
    preferred_provider: Optional[str] = Query(
        None, description="선호하는 공급업체 (선택사항)"
    ),
...
async def check_all_providers_health(
    ...
    try:
        ...
        return {
            ...
            "overall_status": (
                "healthy"
                if all(status["is_healthy"] for status in health_status.values())
                else "degraded"
            ),
            "timestamp": datetime.now().isoformat(),
        }
...
async def get_provider_specific_prices(
    provider_name: str = Path(
        ..., description="공급업체 이름 (tronnrg, energytron 등)"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
...
async def get_system_status(
    user_partner: tuple[User, Optional[Partner]] = Depends(
        require_super_admin_or_partner_admin
    ),
    db: AsyncSession = Depends(get_db),
):
