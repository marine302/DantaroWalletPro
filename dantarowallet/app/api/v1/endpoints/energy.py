...
from app.models.energy_pool import PartnerEnergyPool, EnergyAlert
from app.services.energy_monitoring_service import EnergyMonitoringService
# EnergyPredictionService를 직접 임포트
from app.services.energy_monitoring_service import EnergyPredictionService
from app.schemas.energy import (
    EnergyPoolResponse, EnergyDashboardResponse, 
    EnergyPatternAnalysisResponse, EnergyMonitoringResponse,
    EnergyAnalyticsResponse, EnergyAlertListResponse,
    GlobalEnergyAnalyticsResponse, MessageResponse
)
...
async def monitor_partner_energy(
    ...
    if str(current_partner.id) != str(partner_id):
        raise HTTPException(status_code=403, detail="자신의 에너지 상태만 조회할 수 있습니다")
...
async def get_energy_analytics(
    ...
    if str(current_partner.id) != str(partner_id):
        raise HTTPException(status_code=403, detail="자신의 에너지 분석만 조회할 수 있습니다")
...
async def update_energy_status(
    ...
    # 권한 확인
    if str(current_partner.id) != str(partner_id):
        raise HTTPException(status_code=403, detail="자신의 에너지 상태만 업데이트할 수 있습니다")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"에너지 상태 업데이트 실패: {str(e)}")
...
async def analyze_usage_patterns(
    ...
    # 권한 확인
    if str(current_partner.id) != str(partner_id):
        raise HTTPException(status_code=403, detail="자신의 사용 패턴만 분석할 수 있습니다")
...
async def get_usage_logs(
    ...
    # 권한 확인
    if str(current_partner.id) != str(partner_id):
        raise HTTPException(status_code=403, detail="자신의 사용 로그만 조회할 수 있습니다")
    
    from app.models.energy_pool import PartnerEnergyUsageLog, PartnerEnergyPool
    from sqlalchemy import select, and_, desc
        ...
        "usage_logs": [
            {
                ...
                "energy_unit_price": float(getattr(log, 'energy_unit_price', 0) or 0) if getattr(log, 'energy_unit_price', None) is not None else None,
                "total_cost": float(getattr(log, 'total_cost', 0) or 0) if getattr(log, 'total_cost', None) is not None else None,
...
async def admin_energy_overview(
    db: AsyncSession = Depends(get_db)
):
    """전체 에너지 풀 현황 개요 (슈퍼 어드민 전용)"""
    # TODO: 슈퍼 어드민 권한 확인 로직 추가
    
    from app.models.energy_pool import PartnerEnergyPool, EnergyStatus
    ...
    result = await db.execute(
        select(
            ...
            func.count(PartnerEnergyPool.id).filter(PartnerEnergyPool.status == "sufficient").label("sufficient"),
            func.count(PartnerEnergyPool.id).filter(PartnerEnergyPool.status == "warning").label("warning"),
            func.count(PartnerEnergyPool.id).filter(PartnerEnergyPool.status == "critical").label("critical"),
            func.count(PartnerEnergyPool.id).filter(PartnerEnergyPool.status == "depleted").label("depleted"),
        ...
        "overview": {
            ...
            "energy_totals": {
                ...
                "utilization_rate": round(
                    (1 - (getattr(stats, 'available_energy_sum', 0) / getattr(stats, 'total_energy_sum', 1))) * 100
                    if getattr(stats, 'total_energy_sum', 0) and getattr(stats, 'total_energy_sum', 0) > 0 else 0, 2
                )
            },
