    ...
    try:
        ...
        return {
            ...
            "system": {
                ...
                "success_rate": (
                    (
                        system_metrics.successful_api_calls
                        / system_metrics.total_api_calls
                        * 100
                    )
                    if system_metrics.total_api_calls > 0
                    else 100
                ),
...
async def get_partner_rankings(
    metric: str = Query(
        "performance", description="순위 기준 (performance, volume, revenue)"
    ),
...
async def get_activity_feed(
    ...
    try:
        ...
        activities = [
            {
                ...
                "type": (
                    "partner_created"
                    if i % 3 == 0
                    else "energy_allocated" if i % 3 == 1 else "fee_updated"
                ),
                "title": f"Activity {i}",
        ...
        return {"activities": activities, "total_count": limit, "has_more": False}
...
async def get_quick_stats(
    ...
    try:
        ...
        return {
            ...
            "pending_alerts": len(
                [a for a in alerts if a.severity in ["warning", "critical"]]
            ),
            "avg_response_time": system_metrics.avg_response_time,
