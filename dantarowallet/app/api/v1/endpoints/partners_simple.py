...
@router.get("/")
async def get_partners(skip: int = 0, limit: int = 100, status: Optional[str] = None):
    """파트너 목록 조회"""
    try:
        ...
        return {"items": partners, "total": total, "skip": skip, "limit": limit}
...
async def update_partner(partner_id: str, partner_data: dict):
    """파트너 정보 업데이트"""
    partner_index = next(
        (i for i, p in enumerate(MOCK_PARTNERS) if p["id"] == partner_id), None
    )
