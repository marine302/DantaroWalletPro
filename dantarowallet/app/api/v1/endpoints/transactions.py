...
async def get_transactions(
    ...
    try:
        ...
        for tx in transactions:
            ...
            # 파트너 정보 조회
            partner = (
                db.query(Partner).filter(Partner.id == tx.partner_id).first()
                if tx.partner_id
                else None
            )
            ...
            result.append(
                {
                    ...
                    "updated_at": (lambda ua: ua.isoformat() if ua else None)(
                        getattr(tx, "updated_at", None)
                    ),
                    "user": (
            ...
            result.append(
                {
                    ...
                    "partner": (
                        {"id": partner.id, "name": partner.name} if partner else None
                    ),
                }
            )
...
async def retry_failed_transaction(transaction_id: int, db: Session = Depends(get_db)):
    """
    실패한 거래 재시도
    """
    try:
        ...
        if tx_status != "failed":
            raise HTTPException(
                status_code=400, detail="실패한 거래만 재시도할 수 있습니다"
            )
