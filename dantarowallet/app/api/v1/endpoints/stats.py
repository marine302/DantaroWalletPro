...
async def get_user_stats(db: Session = Depends(get_sync_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        active_users = (
            db.query(User).filter(User.last_activity_at >= thirty_days_ago).count()
        )
        ...
        users_yesterday = db.query(User).filter(User.created_at < yesterday).count()
        ...
        # 주간 성장률 계산
        users_week_ago = db.query(User).filter(User.created_at < seven_days_ago).count()
...
async def get_partner_stats(db: Session = Depends(get_sync_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        thirty_days_ago = datetime.now() - timedelta(days=30)
        partners_month_ago = (
            db.query(Partner).filter(Partner.created_at < thirty_days_ago).count()
        )

        monthly_growth = 0
        if partners_month_ago > 0:
            monthly_growth = (
                (total_partners - partners_month_ago) / partners_month_ago
            ) * 100
        ...
        return {
            ...
            "average_transaction_size": (
                float(total_volume / total_transactions)
...
async def get_dashboard_overview(db: Session = Depends(get_sync_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        # 최근 거래 내역
        recent_transactions = (
            db.query(Transaction).order_by(desc(Transaction.created_at)).limit(10).all()
        )
        ...
        return {
            ...
            "energy": {
                ...
                "usage_rate": (
                    round(((total_energy - available_energy) / total_energy) * 100, 1)
                    if total_energy > 0
                    else 0
                ),
                "status": (
                    "active" if available_energy > (total_energy * 0.2) else "warning"
                ),
            },
            "recent_transactions": [
                {
...
async def get_revenue_analytics(
    period: str = "7d", db: Session = Depends(get_sync_db)
) -> Dict[str, Any]:
    """
    수익 분석 데이터
    """
