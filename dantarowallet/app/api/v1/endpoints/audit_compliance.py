...
async def perform_kyc_check(
    request: ComplianceCheckRequest, db: Session = Depends(get_sync_db)
):
    """KYC 체크 수행"""
    try:
        ...
        result = await compliance_service.perform_kyc_check(
            user_id=request.user_id, check_data=request.check_data
        )
...
async def check_sanctions_list(
    ...
    try:
        ...
        result = await compliance_service.check_sanctions_list(
            name=name, address=address, nationality=nationality
        )
...
async def check_pep_status(
    ...
    try:
        ...
        result = await compliance_service.check_pep_status(
            name=name, position=position, country=country
        )
...
async def detect_anomalies(
    request: AnomalyDetectionRequest, db: Session = Depends(get_sync_db)
):
    """이상 활동 탐지"""
    try:
        ...
        if request.transaction_data:
            ...
            result = await ml_service.detect_transaction_anomalies(
                user_id=request.user_id, transaction_data=request.transaction_data
            )
            return [SuspiciousActivityResponse.from_orm(result)] if result else []
        else:
            # 사용자 행동 이상 탐지
            results = await ml_service.detect_user_behavior_anomalies(
                user_id=request.user_id, time_window_days=request.time_window_days or 30
            )
...
async def get_suspicious_activities(
    ...
    try:
        ...
        activities = (
            query.order_by(SuspiciousActivity.detected_at.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
        ...
        return [
            SuspiciousActivityResponse.from_orm(activity) for activity in activities
        ]
...
async def verify_audit_chain(
    start_id: Optional[int] = None, db: Session = Depends(get_sync_db)
):
    """감사 체인 무결성 검증"""
    try:
        audit_service = AuditService(db)
...
async def get_audit_stats(
    days: int = Query(30, ge=1, le=365), db: Session = Depends(get_sync_db)
):
    """감사 및 컴플라이언스 통계"""
    try:
        ...
        critical_logs = (
            db.query(AuditLog)
            .filter(AuditLog.timestamp >= start_date, AuditLog.severity == "critical")
            .count()
        )
        ...
        total_checks = (
            ...
            .filter(ComplianceCheck.initiated_at >= start_date)
            .count()
        )
        failed_checks = (
            db.query(ComplianceCheck)
