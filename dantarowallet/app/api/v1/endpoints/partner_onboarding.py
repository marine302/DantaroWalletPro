...
async def get_onboarding_detail(
    ...
    try:
        ...
        checklist = await service.get_onboarding_checklist(safe_int(safe_int(onboarding.id)))
        ...
        return OnboardingDetailResponse(
            **onboarding.__dict__,
            ...
            checklist=[OnboardingChecklistResponse(**item.__dict__) for item in checklist],
...
@router.put("/{partner_id}/steps/{step_number}/status", response_model=OnboardingSuccessResponse)
...
async def update_checklist_item(
    ...
    try:
        ...
        # 체크리스트 항목 업데이트
        completed_by = request.completed_by if request.completed_by else safe_str(current_user.email)
