...
async def get_integrated_dashboard_data(
    partner_id: str, db: AsyncSession = Depends(get_db)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if wallet_stats:
            dashboard_data["wallet_overview"]["wallet_count"] = (
                wallet_stats.wallet_count or 0
            )
            dashboard_data["wallet_overview"]["total_balance"] = float(
                wallet_stats.total_balance or 0
            )
        ...
        if transaction_stats:
            dashboard_data["transaction_flow"]["total_count"] = (
                transaction_stats.total_count or 0
            )
            dashboard_data["transaction_flow"]["total_volume"] = float(
                transaction_stats.total_volume or 0
            )
            dashboard_data["transaction_flow"]["avg_amount"] = float(
                transaction_stats.avg_amount or 0
            )

        # 사용자 분석
        user_result = await db.execute(
            select(func.count(User.id).label("total_users")).filter(
        ...
        if user_stats:
            ...
            dashboard_data["user_analytics"]["active_users"] = int(
                total_users * 0.8
            )  # 80% 활성 사용자 가정
            dashboard_data["user_analytics"]["new_users"] = int(
                total_users * 0.05
            )  # 5% 신규 사용자 가정
        ...
        dashboard_data["revenue_metrics"]["commission_earned"] = (
            total_volume * 0.002
        )  # 0.2% 수수료 가정
        dashboard_data["revenue_metrics"]["total_revenue"] = (
            dashboard_data["revenue_metrics"]["commission_earned"] * 1.2
        )
        ...
        logger.error(
            f"Error fetching dashboard data for partner {partner_id}: {str(e)}"
        )
        # Fallback 데이터 반환
        return {
            "wallet_overview": {
