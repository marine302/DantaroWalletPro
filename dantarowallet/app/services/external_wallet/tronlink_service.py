...
class TronLinkService:
    ...
    async def initiate_connection(
        ...
        try:
            ...
            if existing_wallet:
                ...
                # 새 지갑 등록
                new_wallet = await self._create_partner_wallet(
                    partner_id, wallet_address
                )
                return {
                    "status": "connected",
                ...
                wallet_list.append(
                    {
                        ...
                        "last_connected_at": (
                            last_connected.isoformat() if last_connected else None
                        ),
    ...
    async def _get_partner_wallet(
        self, partner_id: int, address: str
    ...
    async def _create_partner_wallet(
        self, partner_id: int, address: str
    ) -> PartnerWallet:
        """새 파트너 지갑 생성"""
