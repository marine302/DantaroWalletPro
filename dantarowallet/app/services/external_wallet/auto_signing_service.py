...
from app.core.exceptions import ValidationError, AuthenticationError, BusinessLogicError
...
class TronLinkAutoSigningService:
    ...
    def __init__(self, db: AsyncSession):
        self.db = db
        self.tron = Tron(network='mainnet' if settings.TRON_NETWORK == 'mainnet' else 'shasta')
    ...
    async def request_account_authorization(
        ...
        try:
            # 서명 검증
            is_valid = await self._verify_wallet_signature(wallet_address, signature, message)
            if not is_valid:
                return {
                    "code": 4001,
            ...
            logger.info(f"TronLink 계정 인증 성공: Partner {partner_id}, Wallet {wallet_address}")
    ...
    async def create_auto_signing_session(
        ...
        try:
            ...
            if not wallet:
                raise ValidationError("등록되지 않은 지갑입니다. 먼저 계정 인증을 완료하세요.")
            ...
            session_config = {
                ...
                "max_amount_per_tx": str(max_amount_per_tx) if max_amount_per_tx else None,
            ...
            # 지갑 업데이트 (자동 서명 활성화)
            await self._update_wallet_auto_signing(wallet.id, {  # type: ignore
                "auto_signing_enabled": True,
                "session_token": session_token,
                "encrypted_config": encrypted_config,
                "session_expires_at": datetime.utcnow() + timedelta(hours=session_duration_hours)
            })
            
            logger.info(f"자동 서명 세션 생성: Partner {partner_id}, Wallet {wallet_address}")
    ...
    async def sign_transaction_with_tronweb(
        ...
        try:
            ...
            signed_txn = await self._create_and_sign_tronweb_transaction(withdrawal, session_config)
            
            # 출금 상태 업데이트
            await self._update_withdrawal_status(withdrawal_id, {
            ...
            logger.info(f"TronWeb 자동 서명 완료: Withdrawal {withdrawal_id}, Hash {signed_txn.get('txn_hash')}")
            ...
            logger.error(f"TronWeb 자동 서명 실패 (Withdrawal {withdrawal_id}): {str(e)}")
        ...
        try:
            ...
            async def sign_single(withdrawal_id: int):
                async with semaphore:
                    try:
                        result = await self.sign_transaction_with_tronweb(withdrawal_id, session_token)
            ...
            logger.info(f"TronLink 배치 자동 서명 완료: {len(results['successful'])}/{results['total']} 성공")
    ...
    async def revoke_auto_signing_session(
        ...
        try:
            ...
            # 지갑에서 자동 서명 비활성화
            wallet = await self._get_partner_wallet(partner_id, session_config["wallet_address"])
            if wallet:
                await self._update_wallet_auto_signing(wallet.id, {  # type: ignore
    ...
    async def get_tronweb_status(
        ...
        try:
            ...
            return {
                ...
                "allowed_addresses_count": len(session_config.get("allowed_addresses", []))
            }
    ...
    async def _verify_wallet_signature(self, wallet_address: str, signature: str, message: str) -> bool:
    ...
    async def _register_or_update_wallet(self, partner_id: int, wallet_address: str) -> PartnerWallet:
        """파트너 지갑 등록 또는 업데이트"""
        # 기존 지갑 확인
        wallet = await self._get_partner_wallet(partner_id, wallet_address)
    ...
    async def _create_and_sign_tronweb_transaction(self, withdrawal: Withdrawal, session_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        TronWeb 스타일 트랜잭션 생성 및 서명
        ...
        try:
                ...
                transaction = {
                    ...
                    "raw_data": {
                        "contract": [{
                            "parameter": {
                                "value": {
                                    "amount": int(float(withdrawal.amount) * 1_000_000),  # TRX to SUN  # type: ignore
                        ...
                        "expiration": int((datetime.utcnow() + timedelta(minutes=10)).timestamp() * 1000),
                        ...
                        "contract": [{
                            "parameter": {
                                "value": {
                                    ...
                                    "owner_address": withdrawal.from_address,
                                    "contract_address": settings.USDT_CONTRACT_ADDRESS or "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"
                                }
                            },
            ...
            signed_transaction = {
                **transaction,
                "signature": ["0x" + hashlib.sha256(f"signature_{txn_hash}".encode()).hexdigest()]
            }
    ...
    async def _get_partner_wallet(self, partner_id: int, wallet_address: str) -> Optional[PartnerWallet]:
        """파트너 지갑 조회"""
    ...
    async def _validate_session(self, partner_id: int, session_token: str) -> Dict[str, Any]:
        """세션 토큰 검증"""
        # 지갑에서 세션 정보 조회
        result = await self.db.execute(
            select(PartnerWallet).where(
    ...
    async def _validate_withdrawal_limits(self, withdrawal: Withdrawal, session_config: Dict[str, Any]):
        """출금 한도 검증"""
        # 건당 한도 검증
        max_amount_per_tx = session_config.get("max_amount_per_tx")
        if max_amount_per_tx and float(withdrawal.amount) > float(max_amount_per_tx):  # type: ignore
            raise ValidationError(f"건당 출금 한도 초과: {withdrawal.amount} > {max_amount_per_tx}")
        ...
        if max_daily_amount:
            ...
            if float(daily_used + withdrawal.amount) > float(max_daily_amount):  # type: ignore
                raise ValidationError(f"일일 출금 한도 초과: {daily_used + withdrawal.amount} > {max_daily_amount}")
    
    async def _update_wallet_auto_signing(self, wallet_id: int, updates: Dict[str, Any]):
        """지갑 자동 서명 설정 업데이트"""
        await self.db.execute(
            update(PartnerWallet)
    ...
    async def _update_withdrawal_status(self, withdrawal_id: int, updates: Dict[str, Any]):
        """출금 상태 업데이트"""
        await self.db.execute(
            update(Withdrawal)
    ...
    async def _update_session_usage(self, session_token: str, amount: Decimal):
        ...
        if wallet and wallet.encrypted_config:
            try:
                ...
                session_config["transaction_count"] = session_config.get("transaction_count", 0) + 1
