...
class SweepService:
    ...
    async def create_sweep_configuration(
        ...
        **config_data
        ...
        try:
            ...
            if not wallet:
                raise ValidationError(f"Destination wallet {destination_wallet_id} not found for partner {partner_id}")
            ...
            if existing:
                raise ValidationError(f"Sweep configuration already exists for partner {partner_id}")
        ...
        **update_data
        ...
        try:
            ...
            if not configuration:
                raise ValidationError(f"Sweep configuration not found for partner {partner_id}")
    ...
    async def queue_sweep(
        ...
        try:
            ...
            if not address or not getattr(address, 'is_active', True) or not getattr(address, 'is_monitored', True):
                raise ValidationError(f"Deposit address {deposit_address_id} not found, inactive, or not monitored")
    ...
    async def get_pending_sweeps(
        ...
        try:
            query = select(SweepQueue).where(
                ...
                selectinload(SweepQueue.deposit_address)
            ).order_by(
                SweepQueue.priority.desc(),
                SweepQueue.created_at.asc()
            ).limit(limit)
            
            # 파트너 필터 적용
            if partner_id:
                query = query.join(UserDepositAddress).join(
                    HDWalletMaster
                ).where(
    ...
    async def process_sweep_queue(
        ...
        try:
            # 큐 항목 조회
            queue_query = select(SweepQueue).where(
                SweepQueue.id == queue_id
            ...
            # Sweep 설정 조회
            config_query = select(SweepConfiguration).join(
                HDWalletMaster, SweepConfiguration.partner_id == HDWalletMaster.partner_id
            ).join(
                UserDepositAddress, UserDepositAddress.hd_wallet_id == HDWalletMaster.id
            ).where(
                UserDepositAddress.id == queue_item.deposit_address_id
            )
            ...
            sweep_log = SweepLog(
                ...
                sweep_type=SweepType.AUTO if queue_item.queue_type.value == QueueType.NORMAL.value else SweepType.EMERGENCY,
            ...
            if tx_hash:
                ...
                # 입금 주소 통계 업데이트
                current_swept = getattr(queue_item.deposit_address, 'total_swept', 0) or 0
                setattr(queue_item.deposit_address, 'total_swept', current_swept + sweep_amount)
                ...
                setattr(configuration, 'total_sweep_amount', current_amount + sweep_amount)
    ...
    async def get_sweep_statistics(
        ...
        try:
            ...
            stats_query = select(
                ...
                SweepLog.__table__.join(SweepConfiguration.__table__)
            ).where(
                and_(
                    SweepConfiguration.partner_id == partner_id,
            ...
            # 활성 주소 수
            active_addresses_query = select(
                func.count(UserDepositAddress.id)
            ).select_from(
            ...
            success_rate = (successful_sweeps / total_sweeps * 100) if total_sweeps > 0 else 0
