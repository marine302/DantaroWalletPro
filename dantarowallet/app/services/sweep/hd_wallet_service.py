...
class HDWalletService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        # 개발 환경에서 암호화키 경고
        if not hasattr(settings, 'WALLET_ENCRYPTION_KEY') or not settings.WALLET_ENCRYPTION_KEY:
            logger.warning("WALLET_ENCRYPTION_KEY not set, using default key for development")
    ...
    async def create_master_wallet(self, partner_id: str) -> HDWalletMaster:
        ...
        try:
            ...
            master_address = tron.generate_address(master_private_key)['base58check_address']
            ...
            logger.error(f"Failed to create master wallet for partner {partner_id}: {e}")
    ...
    async def generate_deposit_address(
        ...
        try:
            ...
            if not force_new:
                existing_query = select(UserDepositAddress).where(
            ...
            # 개인키 암호화
            encrypted_private_key, private_key_salt = self.encryption.encrypt(private_key)
            encrypted_private_key_with_salt = f"{encrypted_private_key}:{private_key_salt}"
            ...
            current_generated = getattr(master_wallet, 'total_addresses_generated', 0) or 0
    ...
    async def get_private_key(
        ...
        try:
            ...
            logger.error(f"Failed to get private key for address {deposit_address_id}: {e}")
    ...
    async def get_master_wallet_stats(
        self, 
        partner_id: str
    ) -> dict:
        """마스터 지갑 통계 조회
        ...
        try:
            ...
            if addr_stats:
                # 딕셔너리로 변환하여 접근
                stats_dict = dict(zip(['total', 'active', 'monitored', 'received', 'swept'], addr_stats))
            ...
            if not collection_address:
                try:
                    ...
                    collection_address = await self.update_collection_address(master_data.id)
                except Exception as e:
                    ...
                    try:
                        await self.db.execute(
                            text("UPDATE hd_wallet_masters SET collection_address = :address WHERE id = :id"),
            ...
            return {
                ...
                "collection_address": collection_address or "",  # None이 반환되지 않도록
            ...
            logger.error(f"Failed to get master wallet stats for partner {partner_id}: {e}")
    ...
    async def update_collection_address(self, master_wallet_id: int) -> str:
        ...
        try:
            ...
            result = await self.db.execute(
                text("SELECT id, encrypted_seed, collection_address FROM hd_wallet_masters WHERE id = :id"),
                {"id": master_wallet_id}
            )
            master_data = result.fetchone()
            ...
            # TRON 마스터 주소 생성
            master_address = self.tron.generate_address(master_private_key)['base58check_address']
            ...
            logger.info(f"Updated collection address for master wallet {master_wallet_id}: {master_address}")
            return master_address
            
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Failed to update collection address for master wallet {master_wallet_id}: {e}")
