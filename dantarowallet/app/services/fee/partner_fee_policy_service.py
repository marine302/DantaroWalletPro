...
class PartnerFeePolicyService:
    ...
    async def create_partner_fee_policy(
        self, partner_id: str, policy_data: PartnerFeePolicyCreate, admin_id: int
    ) -> PartnerFeePolicyResponse:
        """파트너사 수수료 정책을 생성합니다."""
        try:
            # 파트너 존재 확인
            ...
            # 새 정책 생성
            policy = PartnerFeePolicy(partner_id=partner_id, **policy_data.dict())
    ...
    async def update_partner_fee_policy(
        self, partner_id: str, update_data: PartnerFeePolicyUpdate, admin_id: int
    ) -> PartnerFeePolicyResponse:
        ...
        try:
            ...
            old_values = {
                ...
                "max_fee_amount": (
                    safe_str(policy.max_fee_amount)
                    if safe_get_attr(policy, "max_fee_amount")
                    else None
                ),
            }
    ...
    async def create_fee_tier(
        self, partner_id: str, tier_data: FeeTierCreate, admin_id: int
    ) -> FeeTierResponse:
        """구간별 수수료를 생성합니다."""
        try:
            # 파트너 정책 확인
            ...
            # 구간 겹침 검사
            await self._validate_tier_overlap(
                partner_id, tier_data.min_amount, tier_data.max_amount
            )

            # 구간 생성
            tier = FeeTier(partner_fee_policy_id=policy.id, **tier_data.dict())
            ...
            return FeeTierResponse(
                ...
                max_amount=(
                    safe_decimal(tier.max_amount)
                    if safe_get_attr(tier, "max_amount")
                    else None
                ),
    ...
    async def _calculate_fee_by_type(
        ...
        if fee_type == FeeType.FLAT:
            calculated_fee = await self._calculate_flat_fee(
                policy, base_amount, base_rate
            )
        elif fee_type == FeeType.PERCENTAGE:
            calculated_fee = await self._calculate_percentage_fee(
                policy, base_amount, base_rate
            )
        ...
        return {
            ...
            "effective_rate": (
                calculated_fee / base_amount if base_amount > 0 else Decimal("0")
            ),
            "calculation_method": policy.fee_type.value,
            "user_tier_applied": user_tier.tier_name if user_tier else None,
            "metadata": {
                "min_fee_applied": policy.min_fee_amount
                and calculated_fee == policy.min_fee_amount,
                "max_fee_applied": policy.max_fee_amount
                and calculated_fee == policy.max_fee_amount,
        ...
        result = await self.db.execute(
            ...
            .where(
                and_(
                    ...
                    or_(FeeTier.max_amount.is_(None), FeeTier.max_amount >= amount),
                )
            )
            .order_by(FeeTier.min_amount)
            .limit(1)
    ...
    async def _get_partner_policy(self, partner_id: str) -> PartnerFeePolicy:
        ...
        result = await self.db.execute(
            select(PartnerFeePolicy).where(PartnerFeePolicy.partner_id == partner_id)
        )
        policy = result.scalar_one_or_none()
    ...
    async def _validate_tier_overlap(
        ...
        # 기존 구간들과 겹치는지 확인
        query = select(FeeTier).where(FeeTier.partner_fee_policy_id == policy.id)
    ...
    async def _format_policy_response(
        self, policy: PartnerFeePolicy
    ) -> PartnerFeePolicyResponse:
        """정책 응답 포맷팅"""
        return PartnerFeePolicyResponse(
            ...
            max_fee_amount=(
                safe_decimal(policy.max_fee_amount)
                if safe_get_attr(policy, "max_fee_amount")
                else None
            ),
            ...
            promotion_fee_rate=(
                safe_decimal(policy.promotion_fee_rate)
                if safe_get_attr(policy, "promotion_fee_rate")
                else None
            ),
    ...
    async def get_calculation_logs(
        ...
        if calculation_type:
            query = query.where(
                PartnerPolicyCalculationLog.calculation_type == calculation_type
            )
        ...
        return [
            PartnerPolicyCalculationLogResponse(
                ...
                user_id=(
                    safe_int(log.user_id) if safe_get_attr(log, "user_id") else None
                ),
                ...
                admin_id=(
                    safe_int(log.admin_id) if safe_get_attr(log, "admin_id") else None
                ),
            )
            for log in logs
        ]
    ...
    async def delete_partner_fee_policy(self, partner_id: str, admin_id: int) -> bool:
        ...
        try:
            ...
            await self.db.execute(
                select(FeeTier).where(FeeTier.partner_fee_policy_id == policy.id)
            )
