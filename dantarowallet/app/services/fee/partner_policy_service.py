...
class PartnerPolicyService:
    ...
    async def update_user_tier(
        ...
        try:
            ...
            if not tier:
                raise NotFoundError(f"활성 사용자 등급을 찾을 수 없습니다: {partner_id}/{user_id}")
    ...
    async def validate_withdrawal_request(
        ...
        if policy.daily_limit:
            daily_total = await self._get_daily_withdrawal_total(partner_id, user_id, request_time)
        ...
        return {
            ...
            "estimated_delay": policy.batch_delay_minutes if processing_type == "batch" else 0
        }
    ...
    async def validate_energy_usage(
        ...
        if not policy:
            return {
                ...
                "action": "reject" if current_available < required_energy else "proceed",
