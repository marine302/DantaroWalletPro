...
from app.models.fee_config import (
    FeeConfig, FeeHistory, DynamicFeeRule, 
    FeeCalculationLog, FeeRevenueStats
)
from app.schemas.fee import (
    FeeConfigCreate, FeeConfigUpdate, FeeCalculationRequest,
    FeeCalculationResult, DynamicFeeRuleCreate, DynamicFeeRuleUpdate,
    FeeHistoryCreate, PartnerFeeStats, TotalRevenueStats
)
...
class FeeService:
    ...
    async def calculate_fee(
        ...
        try:
            ...
            final_fee = max(
                fee_config.min_fee,
                min(calculated_fee, fee_config.max_fee)
            )
    ...
    async def get_partner_revenue_stats(
        ...
        try:
            ...
            total_result = await self.db.execute(
                select(func.sum(FeeCalculationLog.final_fee))
                .where(
                    and_(
                        FeeCalculationLog.created_at >= start_date,
                        FeeCalculationLog.created_at <= end_date
        ...
        try:
            ...
            total_result = await self.db.execute(
                select(
                    ...
                    func.sum(FeeCalculationLog.final_fee).label("total_fee_collected"),
                    func.avg(FeeCalculationLog.final_fee).label("average_fee"),
                    func.count(func.distinct(FeeCalculationLog.partner_id)).label("partners_count")
                )
                .where(
                    and_(
                        FeeCalculationLog.created_at >= start_date,
    ...
    async def _check_rule_condition(
        ...
        """규칙 조건을 확인합니다."""
        try:
            condition = json.loads(rule.condition_value) if isinstance(rule.condition_value, str) else rule.condition_value
    ...
    async def _create_fee_history(
        self, fee_config_id: int, old_values: Dict[str, Any],
        new_values: Dict[str, Any], changed_by: int, change_reason: str
    ):
        """수수료 변경 이력을 기록합니다."""
        try:
            history = FeeHistory(
    ...
    async def _log_fee_calculation(
        self, request: FeeCalculationRequest, fee_config: FeeConfig,
        base_fee: Decimal, percentage_fee: Decimal, dynamic_multiplier: Decimal,
        final_fee: Decimal, applied_rules: List[str], calculation_details: Dict[str, Any]
    ):
        """수수료 계산 로그를 기록합니다."""
        try:
