...
class SuperAdminFeeService:
    ...
    async def set_partner_fee_config(
        self, partner_id: str, config_data: dict
    ) -> FeeConfigResponse:
        """파트너별 수수료 설정"""
        try:
            # 파트너 확인
            ...
            return FeeConfigResponse(
                ...
                transaction_type=safe_str(
                    safe_get_attr(new_config, "transaction_type")
                ),
                base_fee=safe_decimal(safe_get_attr(new_config, "base_fee")),
                percentage_fee=safe_decimal(
                    safe_get_attr(new_config, "percentage_fee")
                ),
                minimum_fee=safe_decimal(
                    safe_get_attr(new_config, "min_fee")
                ),  # 모델의 필드명은 min_fee
                maximum_fee=safe_decimal(
                    safe_get_attr(new_config, "max_fee")
                ),  # 모델의 필드명은 max_fee
            ...
            self.db.rollback()
            raise HTTPException(
                status_code=500, detail=f"Failed to set fee config: {str(e)}"
            )

    async def calculate_transaction_fee(
        self, partner_id: str, amount: Decimal, transaction_type: str = "transfer"
    ) -> Decimal:
        """거래 수수료 계산"""
        try:
            # 파트너별 수수료 설정 조회
            fee_config = (
                ...
                .order_by(desc(FeeConfig.created_at))
                .first()
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to calculate fee: {str(e)}"
            )
    ...
    async def get_partner_revenue_stats(self, partner_id: str) -> RevenueStats:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to get partner revenue stats: {str(e)}"
            )
    ...
    async def get_total_revenue_stats(self, days: int = 30) -> dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to get total revenue stats: {str(e)}"
            )
    ...
    async def process_settlement(self, partner_id: str, period: str) -> Settlement:
        ...
        try:
            ...
            if period == "monthly":
                ...
                raise HTTPException(
                    status_code=400, detail="Invalid period. Use 'monthly' or 'weekly'"
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to process settlement: {str(e)}"
            )
    ...
    async def get_fee_config_history(self, partner_id: str) -> List[FeeConfigResponse]:
        ...
        try:
            configs = (
                self.db.query(FeeConfig)
                .filter(FeeConfig.partner_id == partner_id)
                .order_by(desc(FeeConfig.created_at))
                .all()
            )
            ...
            return [
                FeeConfigResponse(
                    ...
                    transaction_type=safe_str(
                        safe_get_attr(config, "transaction_type")
                    ),
                    base_fee=safe_decimal(safe_get_attr(config, "base_fee")),
                    percentage_fee=safe_decimal(
                        safe_get_attr(config, "percentage_fee")
                    ),
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to get fee config history: {str(e)}"
            )
    ...
    async def bulk_update_fees(self, updates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for update in updates:
                try:
                    ...
                    if partner_id:  # partner_id가 유효한 경우에만 실행
                        result = await self.set_partner_fee_config(
                            str(partner_id), config_data
                        )
                        results["success"].append(
                            {"partner_id": partner_id, "config_id": result.id}
                        )
                    else:
                        results["errors"].append(
                            {"partner_id": partner_id, "error": "Invalid partner_id"}
                        )

                except Exception as e:
                    results["failed"].append(
                        {"partner_id": update.get("partner_id"), "error": str(e)}
                    )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to bulk update fees: {str(e)}"
            )
    ...
    async def get_fee_analytics(self, days: int = 30) -> dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to get fee analytics: {str(e)}"
            )

    async def configure_dynamic_pricing(
        self, partner_id: str, pricing_rules: dict
    ) -> dict:
        """동적 가격 설정 구성"""
        try:
            partner = self.db.query(Partner).filter(Partner.id == partner_id).first()
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to configure dynamic pricing: {str(e)}"
            )
