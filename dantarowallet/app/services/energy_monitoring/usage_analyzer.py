...
from .utils import safe_bool_check, safe_decimal_to_float, safe_int_conversion
...
class UsageAnalyzer:
    ...
    async def get_energy_analytics(
        ...
        try:
            ...
            # 사용 이력 조회
            usage_logs = await self._get_usage_history(
                safe_int_conversion(energy_pool.id), days
            )

            total_usage = sum(
                safe_decimal_to_float(log.energy_consumed) for log in usage_logs
            )
            ...
            logger.error(
                f"Failed to get energy analytics for partner {partner_id}: {e}"
            )
            return self._empty_analytics_response(partner_id, days)
    ...
    def _empty_analytics_response(self, partner_id: int, days: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "usage_pattern": {"hourly": [], "daily": [], "trend": "no_data"},
            "recommendations": [],
            "generated_at": datetime.utcnow().isoformat(),
        }

    async def _get_usage_history(
        self, energy_pool_id: int, days: int
    ) -> List[PartnerEnergyUsageLog]:
        """에너지 사용 이력 조회"""
    ...
    async def _get_hourly_pattern(
        ...
        return [
            {"hour": h, "usage": usage} for h, usage in sorted(hourly_usage.items())
        ]

    async def _get_daily_pattern(
        self, usage_logs: List[PartnerEnergyUsageLog]
    ) -> List[Dict]:
        """일별 사용 패턴 분석"""
        daily_usage = {}
    ...
    async def _get_recommendations(self, energy_pool: PartnerEnergyPool) -> List[str]:
        ...
        if usage_percentage > 80:
            recommendations.append(
                "에너지 사용량이 높습니다. 추가 TRX 스테이킹을 고려하세요."
            )

        if usage_percentage > 90:
            recommendations.append(
                "에너지가 곧 부족할 수 있습니다. 즉시 충전을 권장합니다."
            )

        frozen_amount = safe_decimal_to_float(energy_pool.frozen_trx_amount)
        if frozen_amount < 100:
            recommendations.append(
                "더 많은 TRX를 스테이킹하여 안정적인 에너지를 확보하세요."
            )
