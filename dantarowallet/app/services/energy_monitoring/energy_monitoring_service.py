...
from app.models.energy_pool import EnergyPrediction, PartnerEnergyPool
...
class EnergyMonitoringService:
        ...
        try:
            ...
            return {
                ...
                "energy_pool": {
                    ...
                    "available_energy": safe_decimal_to_int(
                        energy_pool.available_energy
                    ),
                    "used_energy": safe_decimal_to_int(energy_pool.used_energy),
                    "total_bandwidth": safe_decimal_to_int(energy_pool.total_bandwidth),
                    "available_bandwidth": safe_decimal_to_int(
                        energy_pool.available_bandwidth
                    ),
                    "daily_average_usage": safe_decimal_to_int(
                        energy_pool.daily_average_usage
                    ),
                    "frozen_trx_amount": safe_decimal_to_float(
                        energy_pool.frozen_trx_amount
                    ),
                ...
                "predictions": {
                    "predicted_usage": (
                        safe_decimal_to_int(prediction.predicted_usage)
                        if prediction
                        else 0
                    ),
                    "depletion_estimated_at": (
                        safe_datetime_isoformat(prediction.predicted_depletion)
                        if prediction
                        else None
                    ),
                    "confidence_score": (
                        safe_decimal_to_float(prediction.confidence_score)
                        if prediction
                        else 0
                    ),
                    "hours_remaining": self.pool_manager.calculate_hours_remaining(
                        energy_pool
                    ),
                },
    ...
    async def get_energy_analytics(
        self, partner_id: int, days: int = 30
    ) -> Dict[str, Any]:
    ...
    async def generate_partner_prediction(
        self, partner_id: int
