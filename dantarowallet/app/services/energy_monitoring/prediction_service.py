...
from app.models.energy_pool import PartnerEnergyUsageLog, EnergyPrediction
...
class EnergyPredictionService:
    ...
    async def get_latest_prediction(self, energy_pool_id: int) -> Optional[EnergyPrediction]:
        """최신 예측 조회"""
        try:
            result = await self.db.execute(
            ...
            logger.error(f"Failed to get latest prediction for energy pool {energy_pool_id}: {e}")
            return None
    
    async def predict_depletion_time(self, energy_pool_id: int, current_available: float, daily_avg_usage: float) -> Optional[datetime]:
        """에너지 고갈 시간 예측"""
        try:
    ...
    async def analyze_usage_trend(self, usage_logs: List[PartnerEnergyUsageLog]) -> Dict[str, Any]:
        """사용량 트렌드 분석"""
        try:
            ...
            recent_avg = sum(safe_decimal_to_float(log.energy_consumed) for log in recent_logs) / len(recent_logs)
            
            if not previous_logs:
                return {"trend": "stable", "slope": 0.0, "confidence": 0.5}
            
            previous_avg = sum(safe_decimal_to_float(log.energy_consumed) for log in previous_logs) / len(previous_logs)
            ...
            confidence = min(1.0, len(usage_logs) / 30.0)  # 30일 데이터가 있으면 신뢰도 100%
    ...
    async def predict_peak_usage_times(self, usage_logs: List[PartnerEnergyUsageLog]) -> List[Dict[str, Any]]:
        """피크 사용 시간 예측"""
        try:
            hourly_usage = {}
                ...
                try:
                    if hasattr(log, 'created_at') and log.created_at is not None:
                        ...
                        if hour not in hourly_usage:
                            hourly_usage[hour] = []
                        hourly_usage[hour].append(safe_decimal_to_float(log.energy_consumed))
                except Exception:
                    continue
            ...
            for hour, avg_usage in sorted_hours[:3]:
                peak_times.append({
                    ...
                    "usage_percentage": (avg_usage / max(hourly_averages.values())) * 100 if hourly_averages else 0
                })
