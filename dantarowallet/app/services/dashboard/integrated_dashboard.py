...
class IntegratedDashboard:
    ...
    async def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            # 간단한 캐시 확인
            now = datetime.utcnow()
            if (self.cache_key in self.cache_data and 
                (now - self.cache_data[self.cache_key]['timestamp']).seconds < self.cache_ttl):
            ...
            dashboard_data = {
                "wallet_overview": results[0] if not isinstance(results[0], Exception) else {},
                "transaction_flow": results[1] if not isinstance(results[1], Exception) else {},
                "energy_status": results[2] if not isinstance(results[2], Exception) else {},
                "user_analytics": results[3] if not isinstance(results[3], Exception) else {},
                "revenue_metrics": results[4] if not isinstance(results[4], Exception) else {},
                "risk_alerts": results[5] if not isinstance(results[5], Exception) else [],
                "predictions": results[6] if not isinstance(results[6], Exception) else {},
        ...
        try:
            ...
            for wallet in wallets:
                ...
                wallet_distribution[wallet_type]['wallets'].append({
                    ...
                    'last_activity': safe_get_value(wallet, 'updated_at', datetime.utcnow())
                })
            ...
            if total_balance > 0:
                for wallet_type in wallet_distribution:
                    wallet_distribution[wallet_type]['percentage'] = float(
                        wallet_distribution[wallet_type]['balance'] / total_balance * 100
                    )
            ...
            return {
                ...
                'diversification_index': self.calculate_diversification_index(wallet_distribution)
            }
    ...
    async def get_energy_status(self) -> Dict[str, Any]:
        """에너지 풀 현황"""
        try:
            stmt = select(EnergyPoolModel).where(EnergyPoolModel.partner_id == self.partner_id)
            result = await self.db.execute(stmt)
            ...
            for pool in energy_pools:
                ...
                pool_available = Decimal(str(safe_get_value(pool, 'available_energy', 0)))
            ...
            usage_rate = 0
            if total_energy > 0:
                usage_rate = float((total_energy - available_energy) / total_energy * 100)
            ...
            return {
                ...
                'cost_per_transaction': self.calculate_cost_per_transaction(energy_tx_stats),
    ...
    async def get_user_analytics(self) -> Dict[str, Any]:
        ...
        try:
            stmt = select(
                ...
                func.count(
                    User.id.where(User.last_login >= datetime.utcnow() - timedelta(days=30))
                ).label('active_users'),
                func.count(
                    User.id.where(User.created_at >= datetime.utcnow() - timedelta(days=30))
                ).label('new_users')
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 전체 시스템 점수
            overall_score = (db_health['score'] + wallet_health['score'] + external_health['score']) / 3
