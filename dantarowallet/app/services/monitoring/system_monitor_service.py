...
from app.models.partner import Partner, PartnerApiLog, PartnerDailyStats
from app.models.energy_pool import EnergyPool
from app.models.fee_config import FeeConfig
from app.schemas.monitoring import (
    SystemHealth, PartnerRanking, SystemMetrics,
...
class SystemMonitorService:
    ...
    async def collect_system_metrics(self) -> SystemMetrics:
        ...
        try:
            ...
            energy_utilization = ((total_energy - available_energy) / total_energy * 100) if total_energy > 0 else 0
            
            # API 호출 통계 (최근 24시간)
            yesterday = datetime.utcnow() - timedelta(hours=24)
            total_api_calls = self.db.query(PartnerApiLog).filter(
                PartnerApiLog.created_at >= yesterday
            ).count()
            
            successful_calls = self.db.query(PartnerApiLog).filter(
                and_(
                    PartnerApiLog.created_at >= yesterday,
            ...
            error_rate = ((total_api_calls - successful_calls) / total_api_calls * 100) if total_api_calls > 0 else 0
            
            # 평균 응답시간
            avg_response_time = self.db.query(
                func.avg(PartnerApiLog.response_time_ms)
            ).filter(
                PartnerApiLog.created_at >= yesterday
            ).scalar() or 0
    ...
    async def check_partner_health(self, partner_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            total_calls = self.db.query(PartnerApiLog).filter(
                and_(
            ...
            energy_status = "sufficient" if energy_balance > 1000 else "low" if energy_balance > 100 else "critical"
            ...
            raise HTTPException(status_code=500, detail=f"Failed to check partner health: {str(e)}")
    ...
    async def generate_performance_report(
        ...
        try:
            ...
            # 평균 응답시간
            avg_response_times = [stat.avg_response_time for stat in partner_stats if stat.avg_response_time]
            ...
            for stat in partner_stats:
                ...
                if stat.total_calls and stat.total_calls > 0:
                    error_rate = ((stat.total_calls - (stat.successful_calls or 0)) / stat.total_calls) * 100
                ...
                partner_performance.append({
                    ...
                    "performance_score": max(0, 100 - error_rate - (stat.avg_response_time or 0) / 100)
                })
    ...
    async def get_system_alerts(self, severity: Optional[str] = None) -> List[Alert]:
        ...
        try:
            ...
            # 에너지 부족 알림
            energy_pool = self.db.query(EnergyPool).order_by(desc(EnergyPool.id)).first()
            if energy_pool and energy_pool.available_energy <= energy_pool.critical_threshold:
                alerts.append(Alert(
            ...
            # API 오류율 높은 파트너
            yesterday = datetime.utcnow() - timedelta(hours=24)
            high_error_partners = self.db.query(
                PartnerApiLog.partner_id,
                func.count(PartnerApiLog.id).label("total_calls"),
                func.sum(
                    func.case(
                        (PartnerApiLog.status_code >= 400, 1),
                        else_=0
                    )
                ).label("error_calls")
            ).filter(
                ...
                and_(
                    ...
                    ) / func.count(PartnerApiLog.id)) > 0.1  # 10% 이상 에러율
                )
            ).all()
            
            for partner_stat in high_error_partners:
                error_rate = (partner_stat.error_calls / partner_stat.total_calls) * 100
                partner = self.db.query(Partner).filter(Partner.id == partner_stat.partner_id).first()
    ...
    async def get_partner_rankings(self, metric: str = "performance") -> List[PartnerRanking]:
        """파트너 순위 조회"""
        try:
            # 최근 30일 기준
            ...
            if metric == "performance":
                ...
                for i, stat in enumerate(partner_stats):
                    ...
                    rankings.append(PartnerRanking(
                        rank=i + 1,
                        partner_id=stat.id,
                        partner_name=stat.name,
                ...
                raise HTTPException(status_code=400, detail="Invalid metric. Use 'performance' or 'volume'")
                
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Failed to get partner rankings: {str(e)}")
