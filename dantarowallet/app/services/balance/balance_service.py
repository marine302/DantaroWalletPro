...
class BalanceService:
    ...
    async def get_or_create_balance(self, user_id: int, asset: str = "USDT") -> Dict[str, Any]:
        """잔액 조회 또는 생성"""
        try:
    ...
    async def internal_transfer(self, sender_id: int, receiver_email: str, amount: Decimal, asset: str = "USDT") -> Dict[str, Any]:
        """내부 이체 처리"""
        ...
        if not receiver_balance:
            ...
            await self.db.execute(
                text("INSERT INTO balances (user_id, asset, amount, locked_amount) VALUES (:user_id, :asset, :amount, :locked_amount)"),
                {"user_id": receiver.id, "asset": asset, "amount": float(amount), "locked_amount": 0.0}
            )
        else:
            # 수신자 잔액 업데이트
            await self.db.execute(
                text("UPDATE balances SET amount = amount + :amount WHERE user_id = :user_id AND asset = :asset"),
    ...
    async def get_transaction_history(self, user_id: int, limit: int = 50, offset: int = 0, tx_type: Optional[str] = None, status: Optional[str] = None) -> List[Dict[str, Any]]:
        """거래 내역 조회"""
        return []
    
    async def adjust_balance(self, user_id: int, asset: str, amount: Decimal, reason: str) -> Dict[str, Any]:
        """잔액 조정"""
        try:
            if isinstance(self.db, AsyncSession):
                # AsyncSession으로 직접 SQL 실행
                await self.db.execute(
                    text("INSERT OR REPLACE INTO balances (user_id, asset, amount, locked_amount) VALUES (:user_id, :asset, :amount, :locked_amount)"),
