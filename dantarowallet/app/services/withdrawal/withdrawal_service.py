...
class WithdrawalService:
    ...
    async def create_withdrawal_request(
        ...
        try:
            # 파트너 정책 조회
            policy = (
                self.db.query(PartnerWithdrawalPolicy)
                .filter(PartnerWithdrawalPolicy.partner_id == partner_id)
                .first()
            )
            ...
            if (
                ...
                and policy_type == WithdrawalPolicyType.REALTIME
            ):
            ...
            return {
                ...
                "processing_type": (
                    "realtime" if approval_result["auto_approved"] else "batch"
                ),
    ...
    async def _check_auto_approval_rules(
        self, policy: PartnerWithdrawalPolicy, withdrawal: Withdrawal
    ) -> Dict[str, Any]:
        """자동 승인 규칙 엔진"""
        try:
            ...
            if auto_approve_max and amount > auto_approve_max:
                result["reasons"].append(
                    f"금액이 자동 승인 한도({auto_approve_max})를 초과"
                )
                return result
            ...
            risk_score = await self._calculate_risk_score(
                partner_id, amount, destination
            )
            if risk_score > risk_threshold:
                result["reasons"].append(
                    f"위험 점수({risk_score})가 임계값({risk_threshold})을 초과"
                )
                return result
    ...
    async def _calculate_risk_score(
        self, partner_id: str, amount: Decimal, destination: str
    ) -> int:
        """위험 점수 계산"""
        try:
            risk_score = 0
    ...
    async def _auto_sign_withdrawal(
        self, withdrawal: Withdrawal, policy: PartnerWithdrawalPolicy
    ):
        """TronLink 자동 서명 처리"""
        try:
            # 자동 서명 스크립트 실행
    ...
    async def _request_manual_signature(self, withdrawal: Withdrawal):
        ...
        try:
            safe_str_assign(
                withdrawal, "status", WithdrawalStatus.PENDING_SIGNATURE.value
            )
            # 알림 전송 로직 추가 필요
    ...
    async def _add_to_batch_queue(self, withdrawal: Withdrawal):
        ...
        try:
            ...
            if policy:
                ...
                safe_datetime_assign(
                    withdrawal, "estimated_execution_at", estimated_execution
                )
    ...
    async def _calculate_next_batch_execution(
        self, policy: PartnerWithdrawalPolicy
    ) -> datetime:
        """다음 배치 실행 시간 계산"""
        try:
            execution_time = getattr(policy, "batch_execution_time", "14:00:00")
            execution_days = getattr(
                policy, "batch_execution_days", ["monday", "wednesday", "friday"]
            )
    ...
    async def process_batch_withdrawals(self, partner_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            queued_withdrawals = (
                ...
                .order_by(asc(Withdrawal.created_at))
                .all()
            )
            ...
            # 배치 최적화
            optimized_batches = await self._optimize_withdrawal_batches(
                queued_withdrawals
            )
    ...
    async def _optimize_withdrawal_batches(
    ...
    async def _execute_withdrawal_batch(
        self, batch: List[Withdrawal]
    ) -> Dict[str, Any]:
        """배치 출금 실행"""
        try:
    ...
    def get_withdrawal_history(
        ...
        try:
            withdrawals = (
                self.db.query(Withdrawal)
                .filter(Withdrawal.partner_id == partner_id)
