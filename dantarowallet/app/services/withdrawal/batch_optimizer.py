...
from .utils import safe_decimal, safe_int, safe_str
...
class BatchOptimizer:
    ...
    def _create_optimal_batches(
        self, withdrawals: List[Withdrawal], max_batch_size: int = 20
        ...
        # 금액별로 정렬 (비슷한 금액끼리 묶기)
        sorted_withdrawals = sorted(withdrawals, key=lambda w: safe_decimal(w.amount))
        ...
        for withdrawal in sorted_withdrawals:
            ...
            if len(
                current_batch
            ) >= max_batch_size or current_batch_amount + amount > Decimal("50000"):
    ...
    def _calculate_batch_info(
        self, withdrawals: List[Withdrawal], priority: str
    ) -> Dict[str, Any]:
        """배치 정보를 계산합니다."""
        ...
        for withdrawal in withdrawals:
            ...
            try:
                ...
                if created_at_value and hasattr(created_at_value, "total_seconds"):
                    wait_hours = (
                        datetime.utcnow() - created_at_value
                    ).total_seconds() / 3600
...
class AdvancedBatchOptimizer(OptimizedServiceBase):
    ...
    async def _adjust_for_network_conditions(
        self, batches: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """네트워크 상태에 따른 배치 조정"""
        try:
            ...
            for batch in batches:
                ...
                # 가스비 트렌드 반영
                if (
                    isinstance(gas_trends, dict)
                    and gas_trends.get("trend") == "decreasing"
                ):
                    adjustment_score += 10  # 가스비가 내려가는 추세면 우선순위 상승

                # 노드 성능 반영
                if (
                    isinstance(node_performance, dict)
                    and node_performance.get("avg_response_time", 1000) < 200
                ):
                    adjustment_score += 5  # 빠른 응답시간이면 우선순위 상승

                batch["network_adjustment_score"] = adjustment_score
                batch["final_priority_score"] = (
                    batch.get("priority_score", 0) + adjustment_score
                )
    ...
    async def _check_tron_network_status(self) -> Dict[str, Any]:
        ...
        try:
            # 실제로는 TRON API 호출
            return {"status": "healthy", "tps": 2000, "block_time": 3.0}
        except Exception as e:
    ...
    async def _check_node_performance(self) -> Dict[str, Any]:
        ...
        try:
            # 실제로는 노드 상태 API 호출
            return {"avg_response_time": 150, "success_rate": 99.5, "load": "normal"}
        except Exception as e:
...
class BatchPerformanceMonitor:
    ...
    async def record_batch_execution(
        self, batch_id: str, execution_time: float, success_rate: float, gas_used: float
    ):
        """배치 실행 성능 기록"""
    ...
    def _calculate_trend(self, data: List[Dict[str, Any]]) -> str:
        ...
        recent_avg = sum(d["execution_time"] for d in recent_performance) / len(
            recent_performance
        )
        older_avg = sum(d["execution_time"] for d in older_performance) / len(
            older_performance
        )
