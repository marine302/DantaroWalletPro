...
class ExternalEnergyService:
    ...
    async def get_all_providers(
        ...
        try:
            ...
            for provider in providers:
                logger.info(
                    f"공급자: id={safe_get_value(provider, 'id')}, name={safe_get_value(provider, 'name')}, active={safe_get_value(provider, 'is_active')}"
                )
    ...
    async def get_active_providers(
        ...
        try:
            ...
            for provider in providers:
                logger.info(
                    f"활성 공급자: id={safe_get_value(provider, 'id')}, name={safe_get_value(provider, 'name')}, priority={safe_get_value(provider, 'priority')}"
                )
    ...
    async def get_current_prices(self, session: AsyncSession) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"📊 서비스: 가격 조회 대상 공급자 수: {len(providers)}"
            )  # print 추가
            logger.info(f"가격 조회 대상 공급자 수: {len(providers)}")

            for provider in providers:
                print(
                    f"🔍 공급자 {safe_get_value(provider, 'id')} 가격 조회 중..."
                )  # print 추가
                logger.info(f"공급자 {safe_get_value(provider, 'id')} 가격 조회 중...")

                # 최신 가격 정보 조회
                stmt = (
                    select(ExternalEnergyPriceHistory)
                    .where(ExternalEnergyPriceHistory.provider_id == provider.id)
                    .order_by(desc(ExternalEnergyPriceHistory.recorded_at))
                    .limit(1)
                )
                ...
                print(
                    f"💰 공급자 {safe_get_value(provider, 'id')} 최신 가격 정보: {latest_price}"
                )  # print 추가
                logger.info(
                    f"공급자 {safe_get_value(provider, 'id')} 최신 가격 정보: {latest_price}"
                )
                ...
                if latest_price:
                    ...
                    logger.info(
                        f"provider_type_value: {provider_type_value}, type: {type(provider_type_value)}"
                    )

                    try:
                        # Enum인 경우 .value 속성 사용 (동적 처리)
                        if (
                            hasattr(provider_type_value, "value")
                            and provider_type_value.__class__.__name__ != "str"
                        ):
                    ...
                    logger.info(
                        f"price_value: {price_value}, type: {type(price_value)}"
                    )
                    ...
                    recorded_at_value = safe_get_value(
                        latest_price, "recorded_at", None
                    )
                    logger.info(
                        f"recorded_at_value: {recorded_at_value}, type: {type(recorded_at_value)}"
                    )

                    try:
                        if recorded_at_value and hasattr(
                    ...
                    price_data = {
                        ...
                        "available_amount": safe_get_value(
                            latest_price, "available_amount", 0
                        ),
                    ...
                    print(
                        f"⚠️ 공급자 {safe_get_value(provider, 'id')} 가격 정보 없음"
                    )  # print 추가
                    logger.warning(
                        f"공급자 {safe_get_value(provider, 'id')} 가격 정보 없음"
                    )
    ...
    async def _simulate_purchase(
        self, purchase: ExternalEnergyPurchase, session: AsyncSession
    ):
        """구매 시뮬레이션"""
        try:
            # 구매 실행 시뮬레이션
            ...
            price_per_energy = safe_get_value(
                purchase, "price_per_energy", Decimal("0")
            )
            ...
            setattr(
                purchase,
                "transaction_hash",
                f"mock_tx_{purchase_id}_{datetime.utcnow().timestamp()}",
            )
    ...
    async def get_purchase_history(
        self, session: AsyncSession, provider_id: Optional[int] = None, limit: int = 100
        ...
        try:
            ...
            for purchase in purchases:
                logger.info(
                    f"구매 기록: id={safe_get_value(purchase, 'id')}, energy_amount={safe_get_value(purchase, 'energy_amount')}, status={safe_get_value(purchase, 'status')}"
                )
        ...
        try:
            ...
            for provider in providers:
                ...
                if all_purchases:
                    ...
                    success_rate = (
                        len(successful_purchases) / len(all_purchases)
                    ) * 100
                    ...
                    setattr(
                        ...
                        sum(
                            safe_get_value(p, "energy_amount", 0)
                            for p in successful_purchases
                        ),
                    )
