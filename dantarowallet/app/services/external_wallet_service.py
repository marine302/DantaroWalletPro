...
class ExternalWalletService:
    ...
    def _validate_tron_address(self, address: str) -> bool:
        ...
        try:
            ...
            checksum = decoded[-4:]
            hash_result = hashlib.sha256(hashlib.sha256(address_bytes).digest()).digest()
    ...
    async def _verify_wallet_ownership(self, partner_id: int, wallet_address: str) -> Optional[PartnerWallet]:
        """지갑 소유권 확인"""
        query = select(PartnerWallet).where(
            PartnerWallet.partner_id == partner_id,
            PartnerWallet.wallet_address == wallet_address,
