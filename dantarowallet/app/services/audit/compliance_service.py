...
class ComplianceService:
    ...
    async def perform_kyc_check(
        self, user_id: int, check_data: Dict[str, Any]
    ) -> ComplianceCheck:
        """KYC 체크 수행"""
        try:
            # 기존 KYC 체크 조회
            ...
            safe_dict_assign(
                kyc_check, "provider_response", verification_result["provider_response"]
            )
    ...
    async def perform_aml_check(
        self, user_id: int, transaction_amount: float, transaction_data: Dict[str, Any]
    ) -> ComplianceCheck:
        """AML 체크 수행"""
        try:
            ...
            safe_dict_assign(
                aml_check, "provider_response", verification_result["provider_response"]
            )
    ...
    async def check_sanctions_list(
        ...
        try:
            ...
            safe_str_assign(
                sanctions_check, "risk_level", verification_result["risk_level"]
            )
    ...
    async def check_pep_status(
        self, name: str, position: Optional[str] = None, country: Optional[str] = None
    ) -> ComplianceCheck:
        """PEP (정치적 노출인물) 체크"""
        try:
            pep_check = ComplianceCheck(
                ...
                entity_id=name,
                status="pending",
                check_data={"name": name, "position": position, "country": country},
                initiated_at=datetime.utcnow(),
            )
            ...
            safe_dict_assign(
                pep_check, "provider_response", verification_result["provider_response"]
            )
    ...
    async def _verify_kyc_data(self, check_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        missing_fields = [
            field for field in required_fields if not check_data.get(field)
        ]
    ...
    async def _verify_aml_data(
        self, user_id: int, amount: float, transaction_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """AML 데이터 검증"""
        await asyncio.sleep(0.1)  # 시뮬레이션
    ...
    async def _check_sanctions_database(
        self, name: str, address: Optional[str], nationality: Optional[str]
    ) -> Dict[str, Any]:
        """제재 데이터베이스 체크"""
        await asyncio.sleep(0.1)  # 시뮬레이션
    ...
    async def _check_pep_database(
        self, name: str, position: Optional[str], country: Optional[str]
    ) -> Dict[str, Any]:
        """PEP 데이터베이스 체크"""
        await asyncio.sleep(0.1)  # 시뮬레이션
    ...
    async def get_pending_reviews(self) -> List[ComplianceCheck]:
        ...
        return (
            self.db.query(ComplianceCheck)
            .filter(ComplianceCheck.status == "manual_review")
            .order_by(ComplianceCheck.initiated_at)
            .all()
        )
