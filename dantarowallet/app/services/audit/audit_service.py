...
class AuditService:
    ...
    async def log_event(
        ...
        try:
            ...
            logger.info(f"감사 로그 생성: {event_type.value} - {entity_type}:{entity_id}")
            return audit_log
    ...
    def _calculate_risk_score(self, event_type: AuditEventType, event_data: Dict) -> int:
        """위험 점수 계산"""
        base_scores = {
    ...
    async def verify_audit_chain(self, start_id: Optional[int] = None) -> Dict[str, Any]:
        """감사 체인 무결성 검증"""
        query = self.db.query(AuditLog)
        if start_id:
