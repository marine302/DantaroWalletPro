...
class ConfigValidator:
    ...
    def _create_feature_config(self, partner: Partner) -> Dict[str, Any]:
        """기능 설정 생성"""
        return safe_dict(partner.settings).get("features", {
    ...
    def _update_partner_config(self, partner_id: str, deployment_config: Dict[str, Any]) -> None:
        """파트너 설정 업데이트"""
        update_query = (
            update(Partner)
    ...
    def _validate_features(self, features: Dict[str, Any]) -> List[str]:
        ...
        errors = []
        valid_features = {
            "multi_signature", "cold_wallet", "auto_sweep", 
            "api_access", "webhook_notifications", "2fa_required"
        }
