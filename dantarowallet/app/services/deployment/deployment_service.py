...
class DeploymentService:
    ...
    async def create_partner_instance(
        ...
        """파트너 인스턴스 생성"""
        return await self.deployment_manager.create_partner_instance(
    ...
    async def create_template(
        self, template_data: Dict[str, Any], admin_id: str
    ) -> Dict[str, Any]:
        """새 템플릿 생성"""
        try:
            # 템플릿 데이터 검증
    ...
    async def update_onboarding_step(
        self, partner_id: str, step_id: str, status: str
    ) -> bool:
        """온보딩 단계 상태 업데이트"""
        return await self.partner_onboarding.update_onboarding_step(
            partner_id, step_id, status
        )
    ...
    async def configure_partner_environment(
        ...
        """파트너 환경 설정"""
        return await self.config_validator.configure_partner_environment(
    ...
    async def get_all_deployments(
        ...
        """모든 배포 상태 조회"""
        return await self.deployment_monitor.get_all_deployments(
    ...
    async def full_partner_deployment(
        self, partner_id: str, config_data: Dict[str, Any]
    ) -> DeploymentResult:
        """전체 파트너 배포 프로세스"""
        try:
            ...
            # 4. 파트너 인스턴스 생성
            deployment_result = await self.create_partner_instance(
                partner_id, config_data.get("template_type", "standard")
            )
