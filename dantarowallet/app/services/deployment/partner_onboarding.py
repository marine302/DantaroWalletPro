...
class PartnerOnboarding:
    ...
    async def get_onboarding_steps(self, partner_id: str) -> List[OnboardingStep]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to get onboarding steps: {str(e)}"
            )

    def _create_onboarding_steps(
        self, partner_status: str, onboarding_status: str
    ) -> List[OnboardingStep]:
        """온보딩 단계 생성"""
        return [
            OnboardingStep(
    ...
    def get_onboarding_progress(self, partner_id: str) -> dict:
        ...
        try:
            ...
            completed_steps = len(
                [
                    step
                    for step in onboarding_steps.values()
            ...
            for step_id in step_order:
                if (
                    step_id not in onboarding_steps
                    or onboarding_steps[step_id].get("status") != "completed"
                ):
