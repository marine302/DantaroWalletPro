...
from .utils import safe_dict, safe_int, safe_str
...
class DeploymentMonitor:
    ...
    async def deploy_partner_templates(self, partner_id: str) -> DeploymentStatus:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to deploy templates: {str(e)}"
            )
    ...
    def _update_partner_deployment(
    ...
    def _determine_deployment_status(self, deployment_config: Dict[str, Any]) -> str:
        ...
        deployed_count = sum(
            1 for template in templates.values() if template.get("status") == "deployed"
        )
        failed_count = sum(
            1 for template in templates.values() if template.get("status") == "failed"
        )
    ...
    async def get_all_deployments(
        ...
        try:
            ...
            for partner in partners:
                ...
                deployment_info = {
                    ...
                    "created_at": (
                        ...
                        if hasattr(partner, "created_at")
                        and partner.created_at is not None
                        else None
                    ),
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to get deployments: {str(e)}"
            )
    ...
    def get_deployment_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pending_deployments = (
                ...
                .filter(Partner.onboarding_status.in_(["pending", "deploying"]))
                .count()
            )
            ...
            return {
                ...
                "success_rate": (
                    (active_partners / total_partners * 100)
                    if total_partners > 0
                    else 0
                ),
            }
