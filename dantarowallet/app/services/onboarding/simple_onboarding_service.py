...
class SimpleOnboardingService:
    ...
    async def create_onboarding(
        self, partner_id: str, configuration_data: Dict[str, Any]
    ) -> PartnerOnboarding:
        """온보딩 프로세스 생성"""
        try:
            # 기존 온보딩 확인
    ...
    async def _create_checklist(self, onboarding_id):
        ...
        checklist_items = [
            ...
            (
                ...
                "API 키 안전하게 저장",
                "API 키와 시크릿이 안전하게 보관되는지 확인",
                True,
            ),
            (
                ChecklistCategory.SECURITY,
                "2FA 설정 완료",
                "관리자 계정에 2단계 인증이 설정되었는지 확인",
                True,
            ),
            (
                ChecklistCategory.SECURITY,
                "IP 화이트리스트 설정",
                "허용된 IP 주소만 API 접근 가능하도록 설정",
                False,
            ),
            (
                ChecklistCategory.SECURITY,
                "웹훅 URL HTTPS 사용",
                "알림 웹훅이 HTTPS를 사용하는지 확인",
                True,
            ),
            # 통합 체크리스트
            (
                ChecklistCategory.INTEGRATION,
                "TronLink 지갑 연동",
                "TronLink 지갑이 올바르게 연동되었는지 확인",
                True,
            ),
            (
                ChecklistCategory.INTEGRATION,
                "API 연동 테스트 완료",
                "모든 API 엔드포인트가 정상 작동하는지 테스트",
                True,
            ),
            (
                ChecklistCategory.INTEGRATION,
                "웹훅 수신 테스트",
                "알림 웹훅이 정상적으로 수신되는지 테스트",
                True,
            ),
            (
                ChecklistCategory.INTEGRATION,
                "에러 처리 구현",
                "API 에러 상황에 대한 적절한 처리가 구현되었는지 확인",
                True,
            ),
            # 컴플라이언스 체크리스트
            (
            ...
            (
                ChecklistCategory.COMPLIANCE,
                "개인정보처리방침 동의",
                "개인정보 처리 방침에 동의했는지 확인",
                True,
            ),
            (
                ChecklistCategory.COMPLIANCE,
                "AML/KYC 정책 확인",
                "자금세탁방지 및 고객확인 정책을 이해했는지 확인",
                True,
            ),
            (
                ChecklistCategory.COMPLIANCE,
                "데이터 보관 정책 확인",
                "데이터 보관 및 삭제 정책을 이해했는지 확인",
                False,
            ),
            # 기술 체크리스트
            (
                ChecklistCategory.TECHNICAL,
                "서버 환경 준비",
                "운영 서버 환경이 준비되었는지 확인",
                True,
            ),
