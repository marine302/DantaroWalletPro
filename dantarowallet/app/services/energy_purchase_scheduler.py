...
class EnergyPurchaseScheduler:
            ...
            async with AsyncSessionLocal() as session:
                ...
                for rule in rules:
                    try:
                        ...
                        logger.error(f"규칙 {safe_get_value(rule, 'id', 'unknown')} 처리 오류: {e}")
            
        except Exception as e:
            logger.error(f"스케줄러 주기 실행 오류: {e}")
    
    async def _get_active_rules(self, session: AsyncSession) -> List[EnergyPurchaseRule]:
        """활성 구매 규칙 조회"""
        try:
            stmt = select(EnergyPurchaseRule).where(
    ...
    async def _check_daily_limit(self, rule: EnergyPurchaseRule, session: AsyncSession) -> bool:
        """일일 실행 한도 확인"""
        try:
    ...
    async def _check_trigger_conditions(self, rule: EnergyPurchaseRule, session: AsyncSession) -> bool:
        """트리거 조건 확인"""
        try:
    ...
    async def _check_threshold_trigger(self, rule: EnergyPurchaseRule, session: AsyncSession) -> bool:
        """임계값 트리거 확인"""
        try:
            # 현재 에너지 풀 상태 조회
            ...
            # 전체 에너지 계산
            total_energy = sum(safe_get_value(pool, 'available_energy', 0) for pool in energy_pools)
            total_capacity = sum(safe_get_value(pool, 'total_capacity', 0) for pool in energy_pools)
    ...
    async def _check_prediction_trigger(self, rule: EnergyPurchaseRule, session: AsyncSession) -> bool:
        """예측 트리거 확인"""
        try:
            # 예측 모델을 통한 에너지 수요 예측
            # 현재는 간단한 트렌드 분석
    ...
    async def _execute_auto_purchase(self, rule: EnergyPurchaseRule, session: AsyncSession):
        """자동 구매 실행"""
        try:
            # 구매량 계산
            ...
            # 가격 확인
            provider_prices = await self.external_energy_service.get_current_prices(session)
            provider_price = next(
                (p for p in provider_prices if p['provider_id'] == safe_get_value(best_provider, 'id')), 
                None
            )
            
            if not provider_price:
                logger.warning(f"공급자 {safe_get_value(best_provider, 'name')}의 가격 정보를 찾을 수 없습니다")
                return
            
            # 최대 허용 가격 확인
            max_price = safe_get_value(rule, 'max_price')
            if max_price and provider_price['price_per_energy'] > max_price:
                logger.warning(f"가격이 최대 허용 가격을 초과합니다: {provider_price['price_per_energy']} > {max_price}")
                return
            ...
            if purchase:
                ...
                logger.info(f"자동 구매 완료: {purchase_amount} 에너지, 공급자: {safe_get_value(best_provider, 'name')}")
            
        except Exception as e:
            logger.error(f"자동 구매 실행 오류: {e}")
            await session.rollback()
    
    async def _calculate_purchase_amount(self, rule: EnergyPurchaseRule, session: AsyncSession) -> int:
        """구매량 계산"""
        try:
    ...
    async def _select_best_provider(self, rule: EnergyPurchaseRule, session: AsyncSession) -> Optional[ExternalEnergyProvider]:
        """최적 공급자 선택"""
        try:
            # 기본 조건: 활성 상태
            ...
            if preferred_providers:
                preferred_types = [
                    EnergyProviderType(pt) for pt in preferred_providers
                ]
                stmt = stmt.where(
