...
class EnergyTRONService:
    ...
    async def _make_request(
        ...
        try:
            ...
            logger.error(
                f"EnergyTRON API HTTP error: {e.response.status_code} - {e.response.text}"
            )
    ...
    async def purchase_energy(
        self, request: EnergyPurchaseRequest
    ) -> EnergyPurchaseResponse:
        """에너지 구매 (파트너 API 사용)"""
        try:
            purchase_data = {
            ...
            return EnergyPurchaseResponse(
                ...
                estimated_delivery=datetime.fromisoformat(
                    data.get("estimated_delivery", datetime.now().isoformat())
                ),
    ...
    async def distribute_energy(
        self, source_address: str, allocations: Dict[str, int]
    ) -> Dict[str, Any]:
        """파트너들에게 에너지 분배"""
        try:
            distribution_data = {
            ...
            data = await self._make_request(
                "POST", "/partner/distribute", distribution_data
            )
    ...
    async def get_transaction_status(self, transaction_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            data = await self._make_request(
                "GET", "/partner/transaction/status", params=params
            )
