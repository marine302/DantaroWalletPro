...
from sqlalchemy import select, text, func, Table
    ...
    async def initialize_redis(self):
        ...
        try:
            if not REDIS_AVAILABLE or redis is None:
                logger.warning("Redis 패키지가 설치되지 않았습니다. 캐싱 기능이 제한됩니다.")
                return
    ...
    def cache_query(self, expire_seconds: int = 300):
        ...
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                cache_key = f"query:{func.__name__}:{self.cache_key_generator(*args, **kwargs)}"
        ...
        *relationships
        ...
        try:
            ...
            for relationship in relationships:
                if hasattr(relationship, '_is_collection') and relationship._is_collection:
    ...
    async def analyze_slow_queries(self, session: AsyncSession) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if 'postgresql' in settings.DATABASE_URL:
                ...
                logger.warning("PostgreSQL이 아닌 데이터베이스에서는 슬로우 쿼리 분석이 지원되지 않습니다")
                return []
