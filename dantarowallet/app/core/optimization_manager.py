...
from app.core.background_optimization import OptimizedTaskQueue, BackgroundTask, TaskPriority
...
class IntegratedOptimizationManager:
    ...
    async def _optimize_cpu_usage(self):
        """CPU 사용률 최적화"""
        # 백그라운드 작업 동시 실행 수 감소
        self.task_queue.max_concurrent_tasks = max(1, self.task_queue.max_concurrent_tasks - 1)
        ...
        try:
            ...
            if metrics.throughput > 100:  # 높은 처리량
                ...
                if self.task_queue.max_concurrent_tasks < 10:
                    ...
                    logger.info(f"스케일 업: 동시 작업 수 증가 → {self.task_queue.max_concurrent_tasks}")
            
            elif metrics.throughput < 20:  # 낮은 처리량
                # 리소스 절약
                if self.task_queue.max_concurrent_tasks > 2:
                    self.task_queue.max_concurrent_tasks -= 1
                    logger.info(f"스케일 다운: 동시 작업 수 감소 → {self.task_queue.max_concurrent_tasks}")
    ...
    async def apply_manual_optimization(self, optimization_type: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """수동 최적화 적용"""
        try:
