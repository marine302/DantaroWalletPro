...
class OptimizedTaskQueue:
    ...
    async def _handle_task_failure(self, task: BackgroundTask, error_message: str):
        ...
        if task.retry_count < task.max_retries:
            ...
            logger.info(f"작업 재시도 스케줄링: {task.id} ({task.retry_count}/{task.max_retries})")
    ...
    async def optimize_task_scheduling(self):
        ...
        try:
            ...
            if cpu_usage > 80 or memory_usage > 85:
                ...
                logger.info(f"시스템 부하로 인한 동시 실행 작업 수 감소: {self.max_concurrent_tasks}")
            elif cpu_usage < 50 and memory_usage < 60:
                # 저부하 시 동시 실행 작업 수 증가
                self.max_concurrent_tasks = min(10, self.max_concurrent_tasks + 1)
                logger.info(f"시스템 여유로 인한 동시 실행 작업 수 증가: {self.max_concurrent_tasks}")
            ...
            try:
                ...
                for job_id, job in self.scheduled_jobs.items():
                    ...
                    if current_time >= job['next_run']:
                        ...
                        # 다음 실행 시간 계산
                        if 'interval' in job:
                            job['next_run'] = current_time + timedelta(seconds=job['interval'])
                        elif 'cron' in job:
...
async def optimized_price_update():
    ...
    try:
        # 외부 API 병렬 호출로 최적화
        from app.services.external_energy.external_energy_service import get_external_energy_service
