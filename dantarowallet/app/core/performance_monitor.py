...
class PerformanceMonitor:
    ...
    async def _generate_recommendations(self, metrics: List[PerformanceMetrics]) -> List[str]:
        """성능 개선 권장사항 생성"""
        recommendations = []
        ...
        if avg_cpu > 70:
            recommendations.append("CPU 사용률이 높습니다. 코드 최적화 또는 스케일링을 고려하세요.")
        
        if avg_memory > 80:
            recommendations.append("메모리 사용률이 높습니다. 메모리 누수 확인 또는 캐시 정책 검토가 필요합니다.")
        
        if avg_response > 1.0:
            recommendations.append("응답 시간이 느립니다. 데이터베이스 쿼리 최적화를 검토하세요.")
        
        max_db_connections = max(m.database_connections for m in metrics)
        if max_db_connections > 30:
            recommendations.append("데이터베이스 연결이 많습니다. 연결 풀 설정을 검토하세요.")
