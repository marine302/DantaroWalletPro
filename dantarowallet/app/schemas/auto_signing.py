...
class AutoSigningSessionRequest(BaseModel):
    ...
    session_duration_hours: int = Field(24, ge=1, le=168, description="세션 지속 시간 (시간, 최대 7일)")
    max_amount_per_tx: Optional[Decimal] = Field(None, description="건당 최대 출금 금액")
    max_daily_amount: Optional[Decimal] = Field(None, description="일일 최대 출금 금액")
    allowed_addresses: Optional[List[str]] = Field(None, description="허용된 수신 주소 목록")
...
class BatchResultResponse(BaseModel):
    ...
    successful_withdrawals: List[SuccessfulWithdrawal] = Field(..., description="성공한 출금 목록")
    failed_withdrawals: List[FailedWithdrawal] = Field(..., description="실패한 출금 목록")
