...
class FeeTierCreate(BaseModel):
    ...
    max_amount: Optional[Decimal] = Field(
        None, ge=0, description="최대 금액 (None=무제한)"
    )
...
class PartnerFeePolicyCreate(BaseModel):
    ...
    fee_type: FeeTypeEnum = Field(
        default=FeeTypeEnum.PERCENTAGE, description="수수료 유형"
    )
    base_fee_rate: Decimal = Field(
        default=Decimal("0.001"), ge=0, le=1, description="기본 수수료율"
    )
    min_fee_amount: Decimal = Field(
        default=Decimal("0.1"), ge=0, description="최소 수수료"
    )
    max_fee_amount: Optional[Decimal] = Field(None, ge=0, description="최대 수수료")
    withdrawal_fee_rate: Decimal = Field(
        default=Decimal("0.001"), ge=0, le=1, description="출금 수수료율"
    )
    internal_transfer_fee_rate: Decimal = Field(
        default=Decimal("0"), ge=0, le=1, description="내부 이체 수수료율"
    )
    vip_discount_rates: Optional[Dict[str, float]] = Field(
        None, description="VIP 등급별 할인율"
    )
    promotion_active: bool = Field(default=False, description="프로모션 활성화")
    promotion_fee_rate: Optional[Decimal] = Field(
        None, ge=0, le=1, description="프로모션 수수료율"
    )
    promotion_end_date: Optional[datetime] = Field(None, description="프로모션 종료일")
    platform_share_rate: Decimal = Field(
        default=Decimal("0.3"), ge=0, le=1, description="플랫폼 수수료 분배율"
    )
    fee_tiers: Optional[List[FeeTierCreate]] = Field(
        None, description="구간별 수수료 (TIERED 타입인 경우)"
    )
...
class BatchScheduleConfig(BaseModel):
    ...
    times: List[str] = Field(
        default=["09:00", "15:00", "21:00"], description="처리 시간 (HH:MM)"
    )
    weekdays: List[int] = Field(
        default=[1, 2, 3, 4, 5, 6, 7], description="처리 요일 (1=월, 7=일)"
    )
...
class PartnerWithdrawalPolicyCreate(BaseModel):
    ...
    policy_type: WithdrawalPolicyEnum = Field(
        default=WithdrawalPolicyEnum.HYBRID, description="출금 정책"
    )
    realtime_enabled: bool = Field(default=True, description="실시간 출금 활성화")
    realtime_max_amount: Decimal = Field(
        default=Decimal("1000"), ge=0, description="실시간 최대 금액"
    )
    auto_approve_enabled: bool = Field(default=False, description="자동 승인 활성화")
    auto_approve_max_amount: Decimal = Field(
        default=Decimal("100"), ge=0, description="자동 승인 최대 금액"
    )
    batch_enabled: bool = Field(default=True, description="일괄 출금 활성화")
    batch_schedule: Optional[BatchScheduleConfig] = Field(
        None, description="일괄 처리 스케줄"
    )
    batch_min_amount: Decimal = Field(
        default=Decimal("10"), ge=0, description="일괄 처리 최소 금액"
    )
    daily_limit_per_user: Decimal = Field(
        default=Decimal("10000"), ge=0, description="사용자별 일일 한도"
    )
    daily_limit_total: Decimal = Field(
        default=Decimal("1000000"), ge=0, description="전체 일일 한도"
    )
    single_transaction_limit: Decimal = Field(
        default=Decimal("5000"), ge=0, description="단일 거래 한도"
    )
    whitelist_required: bool = Field(default=False, description="화이트리스트 필수")
    whitelist_addresses: Optional[List[str]] = Field(
        None, description="화이트리스트 주소 목록"
    )
...
class PartnerEnergyPolicyCreate(BaseModel):
    ...
    default_policy: EnergyPolicyEnum = Field(
        default=EnergyPolicyEnum.WAIT_QUEUE, description="기본 대응 정책"
    )
    trx_payment_enabled: bool = Field(default=True, description="TRX 결제 활성화")
    trx_payment_markup: Decimal = Field(
        default=Decimal("0.1"), ge=0, le=1, description="TRX 결제 마크업"
    )
    trx_payment_max_fee: Decimal = Field(
        default=Decimal("20"), ge=0, description="최대 TRX 수수료"
    )
    ...
    vip_priority_levels: Optional[List[VipPriorityLevel]] = Field(
        None, description="VIP 등급별 우선순위"
    )
    energy_saving_enabled: bool = Field(default=False, description="에너지 절약 모드")
    energy_saving_threshold: int = Field(
        default=20, ge=1, le=99, description="절약 모드 임계값 (%)"
    )
...
class UserTierCreate(BaseModel):
    ...
    fee_discount_rate: Decimal = Field(
        default=Decimal("0"), ge=0, le=1, description="수수료 할인율"
    )
    withdrawal_limit_multiplier: Decimal = Field(
        default=Decimal("1.0"), ge=0.1, description="출금 한도 배수"
    )
...
class FeeCalculationRequest(BaseModel):
    ...
    transaction_type: str = Field(
        ..., description="거래 유형 (withdrawal, internal_transfer 등)"
    )
