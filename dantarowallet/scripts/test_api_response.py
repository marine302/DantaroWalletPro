    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                print("✅ API 응답 성공!")
                print(f"   - wallet_overview: {bool(dashboard_data.get('wallet_overview'))}")
                print(f"   - transaction_flow: {bool(dashboard_data.get('transaction_flow'))}")
                print(f"   - energy_status: {bool(dashboard_data.get('energy_status'))}")
                print(f"   - user_analytics: {bool(dashboard_data.get('user_analytics'))}")
