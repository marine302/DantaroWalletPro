...
def create_simple_sample_data():
    ...
    try:
        ...
        # 4. 거래 데이터 생성 (최근 30일)
        cursor.execute("DELETE FROM transactions WHERE description LIKE 'Test transaction%'")
        ...
        for i, pool in enumerate(pool_data):
            ...
            """, (
                ...
                json.dumps({"peak_hours": [9, 10, 11, 14, 15, 16]}),  # peak_usage_hours
        ...
        # 6. 잔액 데이터 생성
        cursor.execute("DELETE FROM balances WHERE user_id IN (SELECT id FROM users WHERE email LIKE 'testuser%')")
        # 사용자별 잔액 생성
