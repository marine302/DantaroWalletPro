...
def test_main_system():
    ...
    try:
        ...
        # 마스터 수집 주소 개인키 암호화
        encrypted_master_key, master_salt = encryption_service.encrypt(
        ...
        for i in range(1, 11):  # 1번부터 10번까지
            ...
            # 사용자 개인키 암호화
            encrypted_user_key, user_salt = encryption_service.encrypt(
        ...
        db.execute(
            text(
                "UPDATE hd_wallet_masters SET last_index = 10, total_addresses_generated = 11 WHERE id = :id"
            ),
            {"id": master_wallet.id},
        )
