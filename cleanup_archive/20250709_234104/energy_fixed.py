...
async def add_to_energy_queue(
    ...
    try:
        energy_service = EnergyPoolService(db)
        queue_id = await energy_service.add_to_queue(current_admin.id, queue_data)
...
async def get_queue_status(
    ...
    try:
        energy_service = EnergyPoolService(db)
        status = await energy_service.get_queue_status(current_admin.id)
...
async def cancel_queue_item(
    ...
    try:
        energy_service = EnergyPoolService(db)
        success = await energy_service.cancel_queue_item(queue_id, current_admin.id)
